"""Field usage analysis: ablate fields to defaults and compare audio features."""
...
def ablate_field(config, field: str):
    defaults = default_internal_config()
    if not hasattr(config, field):
        raise ValueError(f"Unsupported field: {field}")
    return config.model_copy(update={field: getattr(defaults, field)})
...
def run_field_usage(
    *,
    ...
    results_path = output_dir / "field_usage.jsonl"
    ...
    with (
        ...
        for line in handle:
            ...
            for field in fields:
                ablated = ablate_field(internal, field)
                ablated_features = compute_features(
                    ablated, duration=duration, seed=seed + processed, dry_run=dry_run
                )
                delta_features = {
                    key: float(ablated_features[key] - base_features[key]) for key in FEATURE_KEYS
                }
                distance = feature_distance(base_features, ablated_features)
                entry = {
                    "record_id": record_id,
                    "field": field,
                    "base_value": float(getattr(internal, field)),
                    "ablated_value": float(getattr(ablated, field)),
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Ablate config fields and compare audio feature deltas.",
        formatter_class=argparse.ArgumentDefaultsHelpFormatter,
    )
    parser.add_argument("--input", required=True, type=Path, help="Input JSONL with configs")
    parser.add_argument("--output-dir", required=True, type=Path, help="Output directory")
    parser.add_argument("--fields", action="append", default=[], help="Fields to ablate")
