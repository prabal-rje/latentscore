...
def _build_parser(*, include_extra: bool, use_defaults: bool) -> argparse.ArgumentParser:
    ...
    if include_extra:
        ...
        _add_arg(
            parser,
            "--config-no-prompt-caching",
            action="store_true",
            default=False,
            use_defaults=use_defaults,
            help="Disable LiteLLM prompt caching for config generation.",
        )
        _add_arg(
            parser,
            "--dedupe-model",
...
def _default_config() -> dict[str, Any]:
    return {
        ...
        "config_no_prompt_caching": False,
        "model_kwargs": {},
        "config_model_kwargs": {},
...
async def process_split(
    *,
    ...
    config_use_prompt_caching: bool,
    error_rate: float,
    error_seed: int,
    ewma_alpha: float,
    ...
    async def single_config_call(vibe_text: str) -> MusicConfigPromptPayload:
        return await call_music_config(
            ...
            model_kwargs=config_model_kwargs,
            use_prompt_caching=config_use_prompt_caching,
        )
...
async def main_async(args: argparse.Namespace) -> None:
    ...
    config_use_prompt_caching = not args.config_no_prompt_caching
    cache = SqliteCache(args.cache_path)
    await cache.initialize()

    error_seed = args.error_seed if args.error_seed is not None else args.seed

    if config_use_prompt_caching:
        _LOGGER.info("Config prompt caching: ENABLED (system message cached).")
    else:
        _LOGGER.info("Config prompt caching: DISABLED (full input token cost per call).")
    ...
    for split_name in target_splits:
        ...
        await process_split(
            ...
            config_use_prompt_caching=config_use_prompt_caching,
