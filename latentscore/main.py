...
class StreamEvent(BaseModel):
    ...
    index: int | None = None
    item: Streamable | StreamContent | None = None
...
class StreamHooks(BaseModel):
    ...
    on_item_resolve_start: Callable[[int, Streamable | StreamContent], None] | None = None
    on_item_resolve_success: Callable[[int, Streamable | StreamContent], None] | None = None
    on_item_resolve_error: (
        Callable[[int, Streamable | StreamContent, Exception, FallbackInput | None], None] | None
    ) = None
    on_first_config_ready: Callable[[int, Streamable | StreamContent], None] | None = None
...
class FirstAudioSpinner:
    """Spinner helper for first-audio latency."""
...
def stream(
    items: Iterable[Streamable | StreamContent],
    *,
    ...
    try:
        ...
        match items_obj:
            case Streamable():  # type: ignore[reportUnnecessaryComparison]
                raise InvalidConfigError("stream expects an iterable of items")
...
def _coerce_async_items(
    items: Iterable[Streamable | StreamContent] | AsyncIterable[Streamable | StreamContent],
) -> AsyncIterable[Streamable | StreamContent]:
    items_obj: object = items
    match items_obj:
        case Streamable():  # type: ignore[reportUnnecessaryComparison]
            raise InvalidConfigError("astream expects an iterable of items")
        case str():  # type: ignore[reportUnnecessaryComparison]
            raise InvalidConfigError("astream expects an iterable of items")
    ...
    async def _iter() -> AsyncIterator[Streamable | StreamContent]:
...
async def _resolve_fallback(
    ...
    *,
    ...
    if fallback == "embedding":
        ...
        try:
            target = await fallback_model.generate(content)
        except ModelNotAvailableError as exc:
...
async def astream(
    ...
    *,
    ...
    pending_task: asyncio.Task[_ResolvedInstruction | None] | None = None
    try:
        match chunk_seconds:
        ...
        next_index = 0
        queued: _ResolvedInstruction | None = None
        exhausted = False

        async def _next_item() -> tuple[int, Streamable | StreamContent] | None:
            nonlocal next_index
            try:
        ...
        async def _resolve_item(
            idx: int,
            item: Streamable | StreamContent,
            current_config: _MusicConfigInternal | None,
        ) -> _ResolvedInstruction:
            if isinstance(item, Streamable):
                content = item.content
                transition = item.transition_duration
                remaining_chunks = _chunk_count(item.duration, chunk_seconds)
            ...
            _emit_event(hooks, StreamEvent(kind="item_resolve_start", index=idx, item=item))
            try:
                if primary_load is not None and isinstance(content, str):
                    await primary_load.ensure_loaded()
                ...
                _emit_event(
                    hooks,
                    StreamEvent(kind="item_resolve_success", index=idx, item=item),
                )
            except Exception as exc:
                _emit_event(
                    hooks,
        ...
        async def _fetch_next(
            current_config: _MusicConfigInternal | None,
        ) -> _ResolvedInstruction | None:
            next_item = await _next_item()
            if next_item is None:
                return None
            idx, item = next_item
            return await _resolve_item(idx, item, current_config)

        current_remaining: int | None = None
        current_bounded = False

        if current is None:
            first_item = await _next_item()
            if first_item is None:
                _emit_event(hooks, StreamEvent(kind="stream_end"))
                return
            idx, item = first_item
            resolved_item = await _resolve_item(idx, item, current)
            current = resolved_item.target
            current_remaining = resolved_item.remaining_chunks
            current_bounded = resolved_item.remaining_chunks is not None
            if not first_config_ready:
                _emit_event(
                    hooks,
                    StreamEvent(kind="first_config_ready", index=idx, item=resolved_item.item),
                )
                first_config_ready = True

        while True:
            if pending_task is None and queued is None and not exhausted:
                pending_task = asyncio.create_task(_fetch_next(current))
            ...
            if queued is not None and (current_remaining is None or current_remaining <= 0):
                ...
                if transition_steps > 0 and current is not None:  # pyright: ignore[reportUnnecessaryComparison]
                    ...
                    current_remaining = remaining_after_transition
                    current_bounded = queued.remaining_chunks is not None
                else:
                    current = queued.target
                    current_remaining = queued.remaining_chunks
                    current_bounded = queued.remaining_chunks is not None
            ...
            if exhausted and queued is None:
                if current_remaining is not None and current_remaining <= 0:
                    break
                if current_remaining is None and current_bounded:
                    break
            ...
            chunk = await _render_cached_chunk(chunk_cache)
            if not first_audio_chunk:
                _emit_event(hooks, StreamEvent(kind="first_audio_chunk"))
                first_audio_chunk = True
            yield chunk
            ...
            if current_remaining is not None:
                current_remaining -= 1
                if current_remaining <= 0 and queued is None and not exhausted:
